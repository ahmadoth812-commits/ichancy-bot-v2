...
async def start_withdraw(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Entry point for the user to start a withdraw request."""
...
async def ask_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Ask user for phone number after amount validation."""
    try:
    ...
    if balance < amount:
        await update.message.reply_text(
            f"🚫 لا يوجد رصيد كافٍ. رصيدك الحالي: {balance} ل.س"
        )
        return ConversationHandler.END
...
async def confirm_withdraw(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Confirm withdraw and calculate fee."""
    ...
    summary = (
        ...
        f"💸 عمولة الخدمة ({FEE_PERCENT}%): {fee} ل.س\n"
        f"📤 المبلغ الصافي الذي سيتم إرساله: {net_amount} ل.س\n"
        f"📞 الرقم: {phone}\n\n"
...
async def finalize_withdraw(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Save withdraw request in DB and notify admin."""
    ...
    store.add_audit_log("syriatel_withdrawals", tx_id, "pending", "User requested withdrawal")
    ...
    # إشعار الأدمن
    msg = (
        f"🔔 طلب سحب جديد عبر Syriatel Cash\n"
        f"👤 المستخدم: @{q.from_user.username or q.from_user.full_name}\n"
        f"💰 المبلغ: {amount} ل.س\n"
        f"💸 المبلغ الصافي: {net_amount} ل.س\n"
        f"📞 الرقم: {phone}\n"
        f"رقم العملية: {tx_id}"
    )
    kb = InlineKeyboardMarkup([
    ...
    for admin in ADMIN_IDS:
        try:
            await context.bot.send_message(admin, msg, reply_markup=kb)
        except:
            pass
...
async def admin_approve_withdraw(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin approves withdrawal and sends TxID to user."""
    q = update.callback_query
    await q.answer()
...
async def receive_admin_txid(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    store.update_transaction_status("syriatel_withdrawals", tx_id, "approved", txid=txid)
    store.add_audit_log("syriatel_withdrawals", tx_id, "approved", f"Admin confirmed withdrawal with TxID {txid}")
    ...
    if tg_id:
        await context.bot.send_message(
            ...
            f"📤 المبلغ الصافي المرسل: {tx['net_amount']} ل.س\n"
            f"🆔 معرف التحويل: {txid}"
        )
...
async def admin_reject_withdraw(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin rejects withdrawal and asks for reason."""
    q = update.callback_query
    await q.answer()
    if int(q.from_user.id) not in ADMIN_IDS:
        return await q.answer("❌ غير مصرح.")
    tx_id = int(q.data.split(":")[1])
    context.user_data["rejecting_tx"] = tx_id
...
async def receive_reject_reason(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Store rejection reason and notify user."""
    reason = update.message.text.strip()
    tx_id = context.user_data.pop("rejecting_tx", None)
    if not tx_id:
        return await update.message.reply_text("⚠️ لا يوجد عملية لرفضها حالياً.")

    store.update_transaction_status("syriatel_withdrawals", tx_id, "rejected", reason=reason)
    store.add_audit_log("syriatel_withdrawals", tx_id, "rejected", f"Admin rejection reason: {reason}")
    ...
    await update.message.reply_text(f"✅ تم حفظ سبب الرفض للعملية #{tx_id}.")
