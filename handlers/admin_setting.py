...
async def show_settings(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # support being called as callback (query) or command (message)
    if not is_admin(user.id):
        if update.message:
            return await update.message.reply_text("❌ ليس لديك صلاحية الوصول إلى هذه الأوامر.")
        else:
            return await update.callback_query.answer("❌ ليس لديك صلاحية.", show_alert=True)
...
def register_handlers(dp):
    # dp here is Application in your main.py; both add_handler and add_handler(CommandHandler(...)) accepted
